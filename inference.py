...
def TfLiteInference(tflite_model_path, input_image_path):
    ...
    #print(path_parts[-1])
    #print(path_parts[-2])
    dirname = 'results/0601/{}'.format(path_parts[-2])
    isExist = os.path.exists(dirname)
    if not isExist:
        os.mkdir(dirname)
    cv2.imwrite('./results/0601/{}/{}'.format(path_parts[-2], path_parts[-1]), binary_seg_ret * 255)

#print(len([f.name for f in os.scandir('/home/ioannavav/Desktop/workspace/train_set/clips/0601') if f.is_dir()]))
tflite_model_path = '/home/ioannavav/Desktop/workspace/quantized_model.tflite'
list_subfolders_with_paths = [f.name for f in os.scandir('/home/ioannavav/Desktop/workspace/train_set/clips/0601') if f.is_dir() and os.path.exists('results/0601/{}'.format(f.name))==0]
